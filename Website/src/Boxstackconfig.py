...
def detect_stacking_issues(boxes):
    ...
    for i in range(len(boxes) - 1):
        ...
        if overlap_ratio < overlap_threshold:
            issues.append(
                f"Insufficient vertical overlap between Box {i} and Box {i + 1} (overlap ratio: {overlap_ratio:.2f})")
    ...
    for layer, box_indices in layer_y_coordinates.items():
        if len(box_indices) > 1:
            ...
            if max(y_positions) - min(y_positions) > 10:
                issues.append(
                    f"Uneven stacking in layer {layer} (y-coordinate difference: {max(y_positions) - min(y_positions)}px)")
...
def process_frame_with_stacking(frame, pallet_id):
    """Detect and draw stacking configurations in a single frame using YOLOv8."""
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    results = model(frame_rgb, conf=CONF_THRESHOLD, iou=IOU_THRESHOLD)

    if len(results) == 0:
        logging.info("No detections returned by YOLOv8.")
        return frame, []
    ...
    boxes = []
    for (x1, y1, x2, y2), cf, cls_id in zip(xyxys, confs, classes):
        label = class_names[cls_id].lower()
        if label == "box" and cf >= CONF_THRESHOLD:
            boxes.append((int(x1), int(y1), int(x2), int(y2)))
            cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
            cv2.putText(frame, f"{label} ({cf:.2f})", (int(x1), int(y1) - 10),
